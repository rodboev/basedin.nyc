...
def test_render_leaderboard_section_shows_five_rows_before_expanding_by_default() -> None:
    stats = {
        f"user{index:02d}": {"total": 40 - index, "open": 0, "recentCount": 0, "lastCreatedAt": ""}
    ...
    assert 'data-visible-items="5" data-rows-per-item="1"' in html
    assert html.index('data-rank="5"') < html.index("expand-row") < html.index('data-rank="6"')


def test_render_leaderboard_section_caps_display_and_labels_top_20() -> None:
    stats = {
    ...
    assert "Show top 20" in html
    assert 'data-rank="20"' in html
    assert 'data-rank="21"' not in html


def test_render_leaderboard_sections_only_has_per_repo_headings() -> None:
    cache = Cache(
        leaderboards={
    ...
    assert html.startswith('<div class="leaderboard-grid">')
    assert html.count("<h2>") == 0
