...
From the `generator` directory, refresh GitHub data and regenerate the homepage:
...
`../assets/timeline.js` contains the chart behavior. `cachebust.py` updates asset URLs in the publish directory. GitHub Actions assembles `_site` from an explicit list of public files, converts the two PDFs to PNG previews, and publishes only that artifact.
