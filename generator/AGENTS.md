...
Public contribution stats page for Rod Boev. Hosted at `https://basedin.nyc/`.
...
- `template.html`: Jinja2 shell with eight slots; `render_report_page` validates the slot set in both directions.
...
`LEADERBOARD_PAGES_PER_RUN` (700) is a per-run chunk, not a limit. A scan that runs out of budget or loses a page banks its cursor and per-login tallies under the entry's `scan` key and resumes there next run, so progress is never thrown away and a repo can exceed one run's budget without failing. The partial state lives beside the rendered keys rather than replacing them, so a board keeps serving its last complete scan while the next one builds. `LEADERBOARD_MAX_TOTAL_PAGES` (10,000) is only a runaway-cursor backstop; the page that reaches it drops the scan state, reports `failed`, and stamps `scanFailedAt`, which holds the next attempt off for 24h. Failing on that page rather than pausing matters: pausing returned `partial`, the next run found no budget and dropped the scan, and the run after that started again at page one, so a repo past the backstop rebuilt the same doomed scan forever. The chunk is deliberately larger than the biggest active repo (hermes-agent, 651 pages) because a multi-run build stretches the effective refresh period by however many runs it takes, the TTL clock starting at completion.
...
Two rules exist because a resumed scan is not the same thing as a fresh one. It walks from a cursor fixed on its first run, so PRs opened while it was building were never in the ordering it paged through; the board is complete as of the scan's start, not of `cachedAt`. So a multi-run scan sets `scanResumed`, which caps its TTL at the 24h floor instead of the scaled value, and newest-first ordering means the next scan picks up the missed PRs on its first page. And `recentAnchor` records the scan's start separately from `cachedAt`, because `_author_recent_count` has to anchor the author's window on the same instant the board's `recentCount` was counted against or the rate column compares two different weeks.

One asymmetry is left alone deliberately: `_tally_page` has no upper bound while `_author_recent_count` excludes PRs at or after the anchor, so a PR opened during the scan itself counts for the board and not for the author. The window is the scan's own duration, about six minutes for hermes-agent, and closing it would mean bounding the scan on a timestamp it would then have to trust.

`fetch_community_leaderboard` returns a `LeaderboardRefresh` (`fresh`, `refreshed`, `partial`, `failed`, `cooldown`), not a bool. `cooldown` is deliberately not `fresh`: a board held off after the backstop is expired and known-broken, and generate.py warns on it, since reusing `fresh` there would reintroduce the original silence on a 24h clock. The distinction is the point: the old bool collapsed "inside its TTL, nothing to do" and "every attempt aborted" into the same `False`, which is how hermes-agent's board stayed frozen from 2026-06-24 to 2026-08-16 without a single warning. `warn_stale_leaderboards` is the backstop for that whole class, checking every active repo's board age against `LEADERBOARD_STALE_FACTOR` × its TTL at render time regardless of why it is old.
...
The Breakdown cards, bar, and legend in `../index.html` render the **first frame of the load animation**, not the all-time totals. `animateOnLoad` in `../assets/timeline.js` walks `BD_LOAD_RANGES` (`[1, 7, 14, 30, 0]`), lerping between real range states from `bdDisplay(bdStats(r))`, and `updateBreakdown(0)` writes the all-time numbers when it settles. `breakdown_seed` in `core/timeline.py` is a port of `bdStats`/`bdDisplay` at `BD_LOAD_RANGES[0]` and must produce byte-identical values, or the page visibly jumps the moment the animation takes the first frame. `test_breakdown_cards_render_the_load_seed_not_the_all_time_rollup` pins the generated markup to the port; the port itself is only pinned by the unit tests, so change one side and you must change the other.
...
The site-root `index.html` is the generated homepage, titled LGTM Leaderboard. `../pr-stats/index.html` only redirects old links to the homepage. The deployment publishes runtime assets and the three public pages; generator code, caches, tests, and local docs stay out of the Pages artifact.
